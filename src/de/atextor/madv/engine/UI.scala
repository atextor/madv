...
abstract class MenuItem(name: String, description: String) extends Renderable {
...
class Menu[ItemType <: MenuItem](title: String) extends Overlay(pos = Vec2d(100, 50)) {
  ...
  private val stuff: ListBuffer[ItemType] = new ListBuffer()
  
  def addItem(i: ItemType) = {
    if (stuff.isEmpty) selection = 0
    stuff += i
  }
  
  def removeItem(i: ItemType) = {
    stuff -= i
  ...
  def activateSelected: Option[ItemType] = {
    val item: Option[ItemType] = if (selection > -1) Some(stuff(selection)) else None
